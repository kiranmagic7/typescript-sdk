 ...
 */
export class Client {
    /** Static identity + capabilities. `capabilities` is read from {@linkcode legacy} so `registerCapabilities` stays single-source. */
    readonly config: {
     ...
     * `params._meta` keys take precedence.
     */
    private _withMeta(params: Record<string, unknown> | undefined, extra?: Record<string, unknown>): Record<string, unknown> {
        return { ...params, _meta: { ...this._buildMeta(), ...extra, ...(params?._meta as object | undefined) } };
    }

    /**
     * Drains a `sendAndReceive` async iterable: routes `notifications/progress`
     * with the matching token to `opts.onprogress`, drops any other
     * notification (the JSON branch cannot deliver them and the server knows
     * this), parses and returns the first response, throws on JSON-RPC error.
     */
     ...
     * supports `sendAndReceive`. Breaking out of the loop sends
